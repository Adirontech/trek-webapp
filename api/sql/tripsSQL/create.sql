...
BEGIN
    ...
    -- Checking if a session key was found, assigning NULL if not (non-logged in users can still create trips, they just won't be associated with a user account)
    IF NOT FOUND THEN
        v_creator_id := NULL;
    END IF;
