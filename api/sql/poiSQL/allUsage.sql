...
*/
Select p.name AS POI, p.id AS POI_ID, p.type AS type, t.date AS Date, COUNT(t.id) AS Trip_Count, SUM(t.party_size) AS Registered_Visitors, SUM(CASE WHEN t.checked_in THEN t.party_size ELSE 0 END) AS Checked_In_Visitors
FROM POIs p
